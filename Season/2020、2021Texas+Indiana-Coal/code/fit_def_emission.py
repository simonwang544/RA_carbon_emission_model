...
# 参考文件路径
Ref_Path = '/Users/oo/Desktop/carbon_emission_model/2020-florida-Coal.csv'
...
for f in ['P100', 'P500', 'P1000', 'PBig']:  # 包括PBig
    ...
    for i in range(0, len(file_list)):
        ...
        # 按季度分组
        for quarter, group in df0.groupby(pd.Grouper(freq='Q')):
            if group.empty:
                continue
            ...
            y = df['DEF(ton/MW)']
            if len(x) < 2:
                print(f"Skipping unit {unitname}_{quarter.year}_Q{quarter.quarter} due to insufficient data points.")
                continue
            ...
            xlist = x.tolist()
            ylist = y.tolist()
            quarter_str = f"{quarter.year}_Q{quarter.quarter}"
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                if w:
                    for warning in w:
                        print(f"Linear fitting warning for {unitname}_{quarter_str}: {warning.message}")

            plt.plot(x, y, 'bx')
            plt.plot(x1, f_linear(x1), 'r')
            plt.xlabel('Load(MW)')
            plt.ylabel('DEF(ton/MW)')
            plt.title(f'{unitname}_{quarter_str}_Linear')
            ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
            plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Linear.png')
            plt.close()
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                if w:
                    for warning in w:
                        print(f"Quadratic fitting warning for {unitname}_{quarter_str}: {warning.message}")
            ...
            plt.xlabel('Load(MW)')
            plt.ylabel('DEF(ton/MW)')
            plt.title(f'{unitname}_{quarter_str}_Quad')
            ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
            plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Quad.png')
            plt.close()
            ...
            with warnings.catch_warnings(record=True) as w:
                ...
                if w:
                    for warning in w:
                        print(f"Cubic fitting warning for {unitname}_{quarter_str}: {warning.message}")
            ...
            plt.xlabel('Load(MW)')
            plt.ylabel('DEF(ton/MW)')
            plt.title(f'{unitname}_{quarter_str}_Cubic')
            ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
            plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Cubic.png')
            plt.close()

            mse_cubic = mean_squared_error(y, f_cubic(x))
            mape_cubic = mean_absolute_percentage_error(y, f_cubic(x))

            # 其他拟合方法的代码保持不变，只需将 'quarter_str' 替换为 'quarter_str'
            ...
            if p_best is not None:
                ...
                plt.xlabel('Load(MW)')
                plt.ylabel('DEF(ton/MW)')
                plt.title(f'{unitname}_{quarter_str}_Piecewise-Linear')
                ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
                plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Piecewise-Linear.png')
                plt.close()

                f_plinear1 = '%f x+%f' % (p_best[2], p_best[1]-p_best[2]*p_best[0])
                f_plinear2 = '%f x+%f' % (p_best[3], p_best[1]-p_best[3]*p_best[0])
                print(f"Piecewise-Linear Fitting For {unitname}_{quarter_str}: {f_plinear1} and {f_plinear2}")
            ...
            if p_best is not None:
                ...
                plt.xlabel('Load(MW)')
                plt.ylabel('DEF(ton/MW)')
                plt.title(f'{unitname}_{quarter_str}_Piecewise-QL')
                ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
                plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Piecewise-QL.png')
                plt.close()
                ...
                print(f"Piecewise-Quadratic-Linear Fitting For {unitname}_{quarter_str}: {f_pql1} and {f_pql2}")
            ...
            if p_best is None:
                print(f"Skipping unit {unitname}_{quarter_str} due to curve fitting failure.")
                mse_pcq = np.inf
                mape_pcq = np.inf
            else:
                plt.figure()
                ...
                plt.xlabel('Load(MW)')
                plt.ylabel('DEF(ton/MW)')
                plt.title(f'{unitname}_{quarter_str}_Piecewise-CQ')
                ensure_dir(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}')
                plt.savefig(f'/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_fig/{f}/{unitname}_{quarter_str}_Piecewise-CQ.png')
                plt.close()
                ...
                print(f"Piecewise-Cubic-Quad Fitting For {unitname}_{quarter_str}: {f_pcq1} and {f_pcq2}")
            ...
            print(f"For {unitname}_{quarter_str},\n{model_mse} model has minimum MSE = {mse_min}, {model_mape} model has minimum MAPE = {mape_min}\n")

            # 将拟合结果保存到 FIT 和 MSE 列表中
            FIT.append([unitname, quarter.year, quarter.quarter, Aef, Aef_EPA, f_linear, f_quad, f_cubic, (f_plinear1, f_plinear2), (f_pql1, f_pql2), (f_pcq1, f_pcq2)])
            MSE.append([unitname, quarter.year, quarter.quarter, mse_linear, mse_quad, mse_cubic, mse_plinear, mse_pql, mse_pcq])

# 确保MSE列表包含数据
if MSE:
    MSE_df = pd.DataFrame(MSE, columns=['unit', 'year', 'quarter', 'Linear', 'Quadratic', 'Cubic', 'Piecewise-Linear', 'Piecewise-Quad-Linear', 'Piecewise-Cubic-Quad'])
    ensure_dir('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_result')
    MSE_df.to_csv('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_result/fitting_mse_emission.csv', encoding="utf_8_sig", index=False, header=True, mode='w', float_format='%.15g')
else:
    print("No valid MSE data to save.")

if FIT:
    FIT_df = pd.DataFrame(FIT, columns=['unit', 'year', 'quarter', 'AEF', 'AEF_EPA', 'Linear', 'Quadratic', 'Cubic', 'Piecewise-Linear', 'Piecewise-Quad-Linear', 'Piecewise-Cubic-Quad'])
    ensure_dir('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_result')
    FIT_df.to_csv('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/Fitting_result/fitting_curve_emission.csv', encoding="utf_8_sig", index=False, header=True, mode='w', float_format='%.15g')
else:
