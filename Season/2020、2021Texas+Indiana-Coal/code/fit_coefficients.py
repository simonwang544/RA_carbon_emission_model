...
# Save paths
if not os.path.exists('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/CSV'):
    os.makedirs('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/CSV')
csv_save_dir = '/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/CSV'

if not os.path.exists('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/fitting_fig'):
    os.makedirs('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/fitting_fig')
fig_save_dir = '/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/fitting_fig'

if not os.path.exists('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/MSE'):
    os.makedirs('/Users/oo/Desktop/carbon_emission_model/CEMS_plant_filter/time-fitting/MSE')
...
ensure_dir(mse_save_dir)

# Extract coefficients
def extract_coefficients(expression):
    pattern = r"([-+]?[\d.]+) x\*x \+ ([-+]?[\d.]+) x\+ ([-+]?[\d.]+)"
...
for unit in units:
    filtered_data = data[(data['unit'] == unit) & (data['year'].between(2000, 2002))].copy()
    print(f"Processing unit: {unit}")
    print(f"Initial filtered data length: {len(filtered_data)}")

    # Only process rows with non-null 'Piecewise-Quad-Linear'
    filtered_data = filtered_data[pd.notnull(filtered_data['Piecewise-Quad-Linear'])]
    print(f"Filtered data length after removing null 'Piecewise-Quad-Linear': {len(filtered_data)}")

    # Extract coefficients
    filtered_data['coefficients'] = filtered_data['Piecewise-Quad-Linear'].apply(extract_coefficients)
    print(f"Filtered data after extracting coefficients:\n{filtered_data[['Piecewise-Quad-Linear', 'coefficients']].head()}")

    # Check if filtered_data is not empty before splitting coefficients
    if not filtered_data.empty and 'coefficients' in filtered_data.columns:
        filtered_data[['a', 'b', 'k']] = pd.DataFrame(filtered_data['coefficients'].tolist(), index=filtered_data.index)
        print(f"Filtered data with 'a', 'b', 'k' columns:\n{filtered_data[['a', 'b', 'k']].head()}")
    else:
        print(f"Filtered data for unit {unit} is empty or 'coefficients' column is not present.")
        continue

    # Extract coefficients with debug information
    def safe_extract_coefficients(expression):
        coeffs = extract_coefficients(expression)
        if len(coeffs) != 3:
            print(f"Debug: Failed to extract 3 coefficients for expression: {expression}. Extracted: {coeffs}")
        return coeffs

    # Create a new column for debugging
    filtered_data['coefficients'] = filtered_data['Piecewise-Quad-Linear'].apply(lambda x: safe_extract_coefficients(x))
    print(f"Extracted coefficients for unit {unit}:")
    print(filtered_data[['Piecewise-Quad-Linear', 'coefficients']].head())

    # Split coefficients into separate columns
    filtered_data[['a', 'b', 'k']] = pd.DataFrame(filtered_data['coefficients'].tolist(), index=filtered_data.index)

    # Remove rows with NaN values in 'a', 'b', 'k' columns
    filtered_data.dropna(subset=['a', 'b', 'k'], inplace=True)

    if len(filtered_data) < 2:
        print(f"Skipping unit {unit} due to insufficient data points.")
        continue

    # Calculate time in quarters
    filtered_data['month'] = (filtered_data['quarter'] - 1) * 3 + 1
    filtered_data['date'] = pd.to_datetime(filtered_data[['year', 'month']].assign(day=1))
    filtered_data = filtered_data.sort_values(by='date')
    filtered_data['time'] = (filtered_data['date'] - filtered_data['date'].min()).dt.days / 90  # Quarters
    ...
    for coef in ['a', 'b', 'k']:
        ...
        # Save fits to FIT and MSE lists
...
# Save FIT and MSE lists to CSV files
