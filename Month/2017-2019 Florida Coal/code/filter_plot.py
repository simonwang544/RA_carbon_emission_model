...
end_time = datetime.strptime('20191231', '%Y%m%d')
...
for unit_identifier in filtered_data['Unit_Identifier'].drop_duplicates():
    ...
    if len(load_value) >= 30:
        ...
        ##print(f"机组 {unit_identifier} 的负荷最大值：{load_max}，最小值：{load_min}")

        DEF = df_unit['CO2 Mass (short tons)'] / df_unit['Gross Load (MW)']
        Gload = df_unit['Gross Load (MW)']

        ##print(f"DEF 和 Gload 的数量分别为：{len(DEF)},{len(Gload)}")

        if len(DEF.drop_duplicates().dropna()) >= 0.9 * load_max:
            ##print(f"机组 {unit_identifier} 满足条件，生成文件。")
            ...
            plantname = "merged_data"
            plt.figure()
            plt.plot(Gload, DEF, 'o', markersize=1.5)
            plt.xlabel('Load(MW)')
            plt.ylabel('DEF(ton/MW)')
            plt.title(f"{plantname}_unit{unit_identifier}_P{load_max}")

            if type(unit_identifier) == str and not unit_identifier.isalnum():
                unit_identifier = 'xxx' + ''.join(filter(str.isalnum, unit_identifier))

            filename_base = f"{plantname}_unit{unit_identifier}_P{load_max}"
            if 0 < load_max <= 100:
